readGPXt <- function (f = "~/", filter = 1) {
  ...
  if (.Platform$OS.type == "windows") 
        gpsdata <- system(paste(GB, " -", "t", " -i ", "gpx", " -f ", f, filter, " -o unicsv -F -", sep = ","), intern = TRUE, invisible = invisible)

  else gpsdata <- system(paste(GB, " -", "t", " -i ", "gpx", " -f ", f, filter, " -o unicsv -F -", sep = ""), intern = TRUE) 
  gpsdf <- read.csv(con <- textConnection(gpsdata), fill = TRUE)
  close(con)
  gpsdf$Time <- strptime(paste(gpsdf$Date, gpsdf$Time), "%Y/%m/%d %H:%M:%S", tz ="UTC")
  gpsdf
}